  ...
  function createDetailTools({
    ...
    function searchResultDetailGame(result) {
      ...
      return normalizeDetailGame({
        ...
        session,
        difficulty: memory.difficulty || result.difficulty || "normal",
        length: memory.length || result.length || (session === "short" ? "short" : session === "long" ? "long" : "medium"),
        commitment: memory.commitment || result.commitment || (session === "long" ? "high" : "medium"),
        tone: memory.tone || result.tone || "neutral",
        content: memory.content || result.content || "unknown",
        reviewBurden: memory.reviewBurden || result.reviewBurden || (result.sourceId === "manual_unverified" ? "high" : "medium"),
        adultTimeFit: memory.adultTimeFit || result.adultTimeFit || (session === "short" ? "weeknight" : "evening"),
    ...
    function detailGameForTitle(title) {
      ...
      if (memory) {
        return normalizeDetailGame({
          ...
          session: memory.session || "medium",
          difficulty: memory.difficulty || "normal",
          length: memory.length || "unknown",
          commitment: memory.commitment || "medium",
          tone: memory.tone || "neutral",
          content: memory.content || "unknown",
          reviewBurden: memory.reviewBurden || "medium",
          adultTimeFit: memory.adultTimeFit || "evening",
          externalMeta: memory,
